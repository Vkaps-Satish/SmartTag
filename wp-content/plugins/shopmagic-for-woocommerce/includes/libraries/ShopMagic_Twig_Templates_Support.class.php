...
class ShopMagic_Twig_Templates_Support {

	/**
	 * Filesystem Loader class
	 * @var \ShopMagicVendor\Twig_Loader_Filesystem
	 */
	var $fs_loader;

	/**
	 * Database Loader class
	 * @var ShopMagic_Twig_Template_DBLoader
	 */
	var $db_loader;

	/**
	 * Combined Loader class
	 * @var \ShopMagicVendor\Twig_Loader_Chain
	 */
	var $loader;

	/**
	 * Twig class
	 * @var \ShopMagicVendor\Twig_Environment
	 */
	var $twig;
	...
	function __construct() {
		...
		$this->fs_loader = new \ShopMagicVendor\Twig_Loader_Filesystem( $base_path );
		$this->db_loader = new ShopMagic_Twig_Template_DBLoader();
		...
		$this->loader = new \ShopMagicVendor\Twig_Loader_Chain( array( $this->db_loader, $this->fs_loader ) );


		$this->twig = new \ShopMagicVendor\Twig_Environment( $this->loader, array(
			'debug'       => ! empty( $debug ),
			'auto_reload' => true
		) );

		if ( ! empty( $debug ) ) {
			$this->twig->addExtension( new \ShopMagicVendor\Twig_Extension_Debug() );
		}
