...
use ShopMagicVendor\Twig\Source;
/**
 ...
 */
class ModuleNode extends \ShopMagicVendor\Twig\Node\Node
{
    public function __construct(\ShopMagicVendor\Twig_NodeInterface $body, \ShopMagicVendor\Twig\Node\Expression\AbstractExpression $parent = null, \ShopMagicVendor\Twig_NodeInterface $blocks, \ShopMagicVendor\Twig_NodeInterface $macros, \ShopMagicVendor\Twig_NodeInterface $traits, $embeddedTemplates, $name, $source = '')
    {
        if (!$name instanceof \ShopMagicVendor\Twig\Source) {
        ...
        $nodes = ['body' => $body, 'blocks' => $blocks, 'macros' => $macros, 'traits' => $traits, 'display_start' => new \ShopMagicVendor\Twig\Node\Node(), 'display_end' => new \ShopMagicVendor\Twig\Node\Node(), 'constructor_start' => new \ShopMagicVendor\Twig\Node\Node(), 'constructor_end' => new \ShopMagicVendor\Twig\Node\Node(), 'class_end' => new \ShopMagicVendor\Twig\Node\Node()];
        if (null !== $parent) {
            $nodes['parent'] = $parent;
        }
    ...
    protected function compileTemplate(\ShopMagicVendor\Twig\Compiler $compiler)
    {
        if (!$this->getAttribute('index')) {
            $compiler->write('<?php');
        }
        $this->compileClassHeader($compiler);
        if (\count($this->getNode('blocks')) || \count($this->getNode('traits')) || !$this->hasNode('parent') || $this->getNode('parent') instanceof \ShopMagicVendor\Twig\Node\Expression\ConstantExpression || \count($this->getNode('constructor_start')) || \count($this->getNode('constructor_end'))) {
    ...
    protected function compileGetParent(\ShopMagicVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassHeader(\ShopMagicVendor\Twig\Compiler $compiler)
    {
        $compiler->write("\n\n");
        if (!$this->getAttribute('index')) {
    ...
    protected function compileConstructor(\ShopMagicVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileDisplay(\ShopMagicVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassFooter(\ShopMagicVendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('class_end'))->outdent()->write("}\n");
    }
    protected function compileMacros(\ShopMagicVendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('macros'));
    }
    protected function compileGetTemplateName(\ShopMagicVendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getTemplateName()\n", "{\n")->indent()->write('return ')->repr($this->getSourceContext()->getName())->raw(";\n")->outdent()->write("}\n\n");
    }
    protected function compileIsTraitable(\ShopMagicVendor\Twig\Compiler $compiler)
    {
        ...
        if ($traitable) {
            if ($this->getNode('body') instanceof \ShopMagicVendor\Twig\Node\BodyNode) {
                $nodes = $this->getNode('body')->getNode(0);
            } else {
                $nodes = $this->getNode('body');
            }
            if (!\count($nodes)) {
                $nodes = new \ShopMagicVendor\Twig\Node\Node([$nodes]);
            }
            foreach ($nodes as $node) {
                ...
                if ($node instanceof \ShopMagicVendor\Twig\Node\BlockReferenceNode) {
                    continue;
                }
                $traitable = \false;
                break;
    ...
    protected function compileDebugInfo(\ShopMagicVendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getDebugInfo()\n", "{\n")->indent()->write(\sprintf("return %s;\n", \str_replace("\n", '', \var_export(\array_reverse($compiler->getDebugInfo(), \true), \true))))->outdent()->write("}\n\n");
    }
    protected function compileGetSource(\ShopMagicVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileGetSourceContext(\ShopMagicVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileLoadTemplate(\ShopMagicVendor\Twig\Compiler $compiler, $node, $var)
    {
...
\class_alias('ShopMagicVendor\\Twig\\Node\\ModuleNode', 'ShopMagicVendor\\Twig_Node_Module');
