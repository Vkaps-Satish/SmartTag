 ...
 */
namespace ShopMagicVendor\Twig\Test;
 ...
 */
abstract class IntegrationTestCase extends \ShopMagicVendor\PHPUnit\Framework\TestCase
{
    /**
     * @return string
     */
    ...
    protected function doIntegrationTest($file, $message, $condition, $templates, $exception, $outputs)
    {
        ...
        $loader = new \ShopMagicVendor\Twig\Loader\ArrayLoader($templates);
        foreach ($outputs as $i => $match) {
            $config = \array_merge(['cache' => \false, 'strict_variables' => \true], $match[2] ? eval($match[2] . ';') : []);
            $twig = new \ShopMagicVendor\Twig\Environment($loader, $config);
            ...
            try {
                ...
                throw new \ShopMagicVendor\Twig\Error\Error(\sprintf('%s: %s', \get_class($e), $e->getMessage()), -1, null, $e);
            }
            try {
                ...
                $e = new \ShopMagicVendor\Twig\Error\Error(\sprintf('%s: %s', \get_class($e), $e->getMessage()), -1, null, $e);
                $output = \trim(\sprintf('%s: %s', \get_class($e), $e->getMessage()));
            }
            if (\false !== $exception) {
                list($class) = \explode(':', $exception);
                $constraintClass = \class_exists('ShopMagicVendor\\PHPUnit\\Framework\\Constraint\\Exception') ? 'PHPUnit\\Framework\\Constraint\\Exception' : 'PHPUnit_Framework_Constraint_Exception';
...
\class_alias('ShopMagicVendor\\Twig\\Test\\IntegrationTestCase', 'ShopMagicVendor\\Twig_Test_IntegrationTestCase');
