 ...
 */
class Lexer implements \ShopMagicVendor\Twig_LexerInterface
{
    protected $tokens;
    protected $code;
    protected $cursor;
    ...
    const PUNCTUATION = '()[]{}?:.,|';
    public function __construct(\ShopMagicVendor\Twig\Environment $env, array $options = [])
    {
        $this->env = $env;
    ...
    {
        if (!$code instanceof \ShopMagicVendor\Twig\Source) {
            ...
            $this->source = new \ShopMagicVendor\Twig\Source($code, $name);
        } else {
            $this->source = $code;
        }
        ...
        $this->pushToken(\ShopMagicVendor\Twig\Token::EOF_TYPE);
        if (!empty($this->brackets)) {
            list($expect, $lineno) = \array_pop($this->brackets);
            throw new \ShopMagicVendor\Twig\Error\SyntaxError(\sprintf('Unclosed "%s".', $expect), $lineno, $this->source);
        }
        if ($mbEncoding) {
            \mb_internal_encoding($mbEncoding);
        }
        return new \ShopMagicVendor\Twig\TokenStream($this->tokens, $this->source);
    }
    protected function lexData()
    {
        // if no matches are left we return the rest of the template as simple text token
        if ($this->position == \count($this->positions[0]) - 1) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::TEXT_TYPE, \substr($this->code, $this->cursor));
        ...
        $this->pushToken(\ShopMagicVendor\Twig\Token::TEXT_TYPE, $text);
        ...
        switch ($this->positions[1][$this->position][0]) {
            ...
            case $this->options['tag_block'][0]:
                ...
                if (\preg_match($this->regexes['lex_block_raw'], $this->code, $match, 0, $this->cursor)) {
                    ...
                    $this->pushToken(\ShopMagicVendor\Twig\Token::BLOCK_START_TYPE);
                    $this->pushState(self::STATE_BLOCK);
                    $this->currentVarBlockLine = $this->lineno;
                }
                break;
            case $this->options['tag_variable'][0]:
                $this->pushToken(\ShopMagicVendor\Twig\Token::VAR_START_TYPE);
    ...
    protected function lexBlock()
    {
        if (empty($this->brackets) && \preg_match($this->regexes['lex_block'], $this->code, $match, 0, $this->cursor)) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::BLOCK_END_TYPE);
            $this->moveCursor($match[0]);
            $this->popState();
        } else {
            $this->lexExpression();
    ...
    protected function lexVar()
    {
        if (empty($this->brackets) && \preg_match($this->regexes['lex_var'], $this->code, $match, 0, $this->cursor)) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::VAR_END_TYPE);
            $this->moveCursor($match[0]);
            $this->popState();
        } else {
            $this->lexExpression();
    ...
    protected function lexExpression()
    {
        ...
        if (\preg_match('/\\s+/A', $this->code, $match, 0, $this->cursor)) {
            ...
            if ($this->cursor >= $this->end) {
                throw new \ShopMagicVendor\Twig\Error\SyntaxError(\sprintf('Unclosed "%s".', self::STATE_BLOCK === $this->state ? 'block' : 'variable'), $this->currentVarBlockLine, $this->source);
            }
        }
        // arrow function
        if ('=' === $this->code[$this->cursor] && '>' === $this->code[$this->cursor + 1]) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::ARROW_TYPE, '=>');
            $this->moveCursor('=>');
        } elseif (\preg_match($this->regexes['operator'], $this->code, $match, 0, $this->cursor)) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::OPERATOR_TYPE, \preg_replace('/\\s+/', ' ', $match[0]));
            $this->moveCursor($match[0]);
        } elseif (\preg_match(self::REGEX_NAME, $this->code, $match, 0, $this->cursor)) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::NAME_TYPE, $match[0]);
            $this->moveCursor($match[0]);
            ...
            $this->pushToken(\ShopMagicVendor\Twig\Token::NUMBER_TYPE, $number);
            $this->moveCursor($match[0]);
            ...
            if (\false !== \strpos('([{', $this->code[$this->cursor])) {
                ...
                if (empty($this->brackets)) {
                    throw new \ShopMagicVendor\Twig\Error\SyntaxError(\sprintf('Unexpected "%s".', $this->code[$this->cursor]), $this->lineno, $this->source);
                }
                list($expect, $lineno) = \array_pop($this->brackets);
            ...
            $this->pushToken(\ShopMagicVendor\Twig\Token::PUNCTUATION_TYPE, $this->code[$this->cursor]);
            ++$this->cursor;
        } elseif (\preg_match(self::REGEX_STRING, $this->code, $match, 0, $this->cursor)) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::STRING_TYPE, \stripcslashes(\substr($match[0], 1, -1)));
            $this->moveCursor($match[0]);
            ...
            throw new \ShopMagicVendor\Twig\Error\SyntaxError(\sprintf('Unexpected character "%s".', $this->code[$this->cursor]), $this->lineno, $this->source);
        }
    }
    protected function lexRawData($tag)
    {
        ...
        if (!\preg_match(\str_replace('%s', $tag, $this->regexes['lex_raw_data']), $this->code, $match, \PREG_OFFSET_CAPTURE, $this->cursor)) {
            throw new \ShopMagicVendor\Twig\Error\SyntaxError(\sprintf('Unexpected end of file: Unclosed "%s" block.', $tag), $this->lineno, $this->source);
        }
    ...
    protected function lexComment()
    {
        if (!\preg_match($this->regexes['lex_comment'], $this->code, $match, \PREG_OFFSET_CAPTURE, $this->cursor)) {
            throw new \ShopMagicVendor\Twig\Error\SyntaxError('Unclosed comment.', $this->lineno, $this->source);
        }
    ...
    protected function lexString()
    {
        if (\preg_match($this->regexes['interpolation_start'], $this->code, $match, 0, $this->cursor)) {
            ...
            $this->pushToken(\ShopMagicVendor\Twig\Token::INTERPOLATION_START_TYPE);
            $this->moveCursor($match[0]);
            $this->pushState(self::STATE_INTERPOLATION);
        } elseif (\preg_match(self::REGEX_DQ_STRING_PART, $this->code, $match, 0, $this->cursor) && \strlen($match[0]) > 0) {
            $this->pushToken(\ShopMagicVendor\Twig\Token::STRING_TYPE, \stripcslashes($match[0]));
            $this->moveCursor($match[0]);
        } elseif (\preg_match(self::REGEX_DQ_STRING_DELIM, $this->code, $match, 0, $this->cursor)) {
            list($expect, $lineno) = \array_pop($this->brackets);
    ...
    protected function lexInterpolation()
    {
        ...
        if ($this->options['interpolation'][0] === $bracket[0] && \preg_match($this->regexes['interpolation_end'], $this->code, $match, 0, $this->cursor)) {
            ...
            $this->pushToken(\ShopMagicVendor\Twig\Token::INTERPOLATION_END_TYPE);
            $this->moveCursor($match[0]);
            $this->popState();
        } else {
            $this->lexExpression();
    ...
    protected function pushToken($type, $value = '')
    {
        ...
        if (\ShopMagicVendor\Twig\Token::TEXT_TYPE === $type && '' === $value) {
            return;
        }
        $this->tokens[] = new \ShopMagicVendor\Twig\Token($type, $value, $this->lineno);
    }
...
\class_alias('ShopMagicVendor\\Twig\\Lexer', 'ShopMagicVendor\\Twig_Lexer');
